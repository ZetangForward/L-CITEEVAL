...
if __name__ == "__main__":
    ...
    refer_path = "dataset/{}/{}.json".format(args.exp, args.task)
    ...
    with open(path, 'r') as f:
        ...
        for i in range(len(temp)):
            ...
            for d in docs:
                if type(d) == dict:
                    docs_lst.append(d['text'])
                else:
                    docs_lst.append(d)

            temp[i]['docs'] = docs_lst
            length = len(tokenizer.encode(" ".join(docs_lst)))
            for length_id in range(len(length_lst)):
                low_bound = length_lst[length_id-1] if length_id != 0 else 0
                upper_bound = length_lst[length_id]
                if args.exp == 'main' and args.task == 'multi_news' and upper_bound > 40000: 
                    len_samples[40000].append(temp[i])
                    break
                if low_bound < len(tokenizer.encode(" ".join(docs_lst))) < upper_bound:
